...
use super::{
    fuzz_instructions_generator::ProgramAccount,
    types::{Idl, IdlAccount, IdlInstruction},
};
...
pub(crate) fn get_instruction_ixops(
    ...
    idl.instructions
        ...
        .fold(Vec::new(), |mut instructions_ixops_impl, instruction| {
            ...
            let accounts = get_accounts(instruction, program_accounts);

            let data = get_data(instruction, program_accounts);
    ...
    for account in &instruction.accounts {
        match account {
            super::types::IdlAccountItem::IdlAccount(idl_account) => {
                let account_impl = process_single_account(idl_account, program_accounts);
...
fn process_single_account(
    ...
    let ident_long = format_ident!("{}", account_name);
