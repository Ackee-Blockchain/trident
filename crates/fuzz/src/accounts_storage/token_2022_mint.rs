...
use crate::accounts_storage::{
    account_storage::AccountsStorage, 
    ParamValue
};
...
impl AccountsStorage {
    ...
    pub(crate) fn create_token_2022_mint(
        ...
        let actual_extensions: Vec<ExtensionType> = extension_params
            ...
            .map(|params| {
                params.iter().map(|param| match param {
                    ...
                    ParamValue::InterestBearingConfig(_) => ExtensionType::InterestBearingConfig,
                    ParamValue::TransferFeeConfig(_) => ExtensionType::TransferFeeConfig,
                    ParamValue::ConfidentialTransferMint(_) => ExtensionType::ConfidentialTransferMint,
                    ParamValue::ConfidentialTransferFeeConfig(_) => ExtensionType::ConfidentialTransferFeeConfig,
        ...
        let mint_size = if actual_extensions.is_empty() {
            ...
            ExtensionType::try_calculate_account_len::<spl_token_2022::state::Mint>(&actual_extensions)
        ...
        if !actual_extensions.is_empty() {
            ...
            let mut state = StateWithExtensionsMut::<spl_token_2022::state::Mint>::unpack_uninitialized(&mut data)
            ...
            for extension_type in &actual_extensions {
                ...
                ExtensionType::MetadataPointer => {
                    ...
                    let extension = state.init_extension::<MetadataPointer>(true)
                    ...
                    if let Some(param) = extension_params.as_ref().and_then(|params| {
                        params.iter().find(|p| matches!(p, ParamValue::MetadataPointer(_)))
                    }) {
                        if let ParamValue::MetadataPointer(metadata_config) = param {
                            // Set authority
                            extension.authority = if let Some(authority) = metadata_config.authority {
                                OptionalNonZeroPubkey::try_from(Some(Token2022Pubkey::new_from_array(authority.to_bytes())))
                                    .expect("Invalid metadata pointer authority")
                            } else {
                                OptionalNonZeroPubkey::default()
                            };
                            eprintln!("Setting MetadataPointer authority: {:?}", metadata_config.authority);
                            
                            // Set metadata_address
                            extension.metadata_address = if let Some(metadata_address) = metadata_config.metadata_address {
                                OptionalNonZeroPubkey::try_from(Some(Token2022Pubkey::new_from_array(metadata_address.to_bytes())))
                                    .expect("Invalid metadata address")
                            } else {
                                OptionalNonZeroPubkey::default()
                            };
                            eprintln!("Setting MetadataPointer metadata_address: {:?}", metadata_config.metadata_address);
                        }
                    } else {
                ...
                ExtensionType::GroupMemberPointer => {
                    ...
                    let extension = state.init_extension::<GroupMemberPointer>(true)
                        .expect("Failed to init group member pointer extension");
                    
                    // Get the struct directly from ParamValue
                    if let Some(param) = extension_params.as_ref().and_then(|params| {
                        params.iter().find(|p| matches!(p, ParamValue::GroupMemberPointer(_)))
                    }) {
                        if let ParamValue::GroupMemberPointer(group_config) = param {
                            // Set authority
                            extension.authority = if let Some(authority) = group_config.authority {
                                eprintln!("Setting GroupMemberPointer authority: {}", authority);
                                OptionalNonZeroPubkey::try_from(Some(Token2022Pubkey::new_from_array(authority.to_bytes())))
                                    .expect("Invalid group member pointer authority")
                            } else {
                                eprintln!("Setting GroupMemberPointer authority to default (None)");
                                OptionalNonZeroPubkey::default()
                            };
                            
                            // Set member_address
                            extension.member_address = if let Some(member_address) = group_config.member_address {
                                eprintln!("Setting GroupMemberPointer member_address: {}", member_address);
                                OptionalNonZeroPubkey::try_from(Some(Token2022Pubkey::new_from_array(member_address.to_bytes())))
                ...
                ExtensionType::TransferHook => {
                    ...
                    let extension = state.init_extension::<TransferHook>(true)
                        .expect("Failed to init transfer hook extension");
                    
                    // Get the struct directly from ParamValue
                    if let Some(param) = extension_params.as_ref().and_then(|params| {
                        params.iter().find(|p| matches!(p, ParamValue::TransferHook(_)))
                    }) {
                        if let ParamValue::TransferHook(hook_config) = param {
                            ...
                            // Set program_id
                            extension.program_id = if let Some(program_id) = hook_config.program_id {
                                eprintln!("Setting TransferHook program_id: {}", program_id);
                                OptionalNonZeroPubkey::try_from(Some(Token2022Pubkey::new_from_array(program_id.to_bytes())))
                ...
                ExtensionType::MintCloseAuthority => {
                    ...
                    let extension = state.init_extension::<MintCloseAuthority>(true)
                        .expect("Failed to init mint close authority extension");
                    
                    // Get the struct directly from ParamValue
                    if let Some(param) = extension_params.as_ref().and_then(|params| {
                        params.iter().find(|p| matches!(p, ParamValue::MintCloseAuthority(_)))
                    }) {
                        if let ParamValue::MintCloseAuthority(close_config) = param {
                            // Set close_authority
                            extension.close_authority = if let Some(close_authority) = close_config.close_authority {
                                eprintln!("Setting MintCloseAuthority close_authority: {}", close_authority);
                                OptionalNonZeroPubkey::try_from(Some(Token2022Pubkey::new_from_array(close_authority.to_bytes())))
                ...
                ExtensionType::PermanentDelegate => {
                    ...
                    let extension = state.init_extension::<PermanentDelegate>(true)
                        .expect("Failed to init permanent delegate extension");
                    
                    // Get the struct directly from ParamValue
                    if let Some(param) = extension_params.as_ref().and_then(|params| {
                        params.iter().find(|p| matches!(p, ParamValue::PermanentDelegate(_)))
                    }) {
                        if let ParamValue::PermanentDelegate(delegate_config) = param {
                            // Set delegate
                            extension.delegate = if let Some(delegate) = delegate_config.delegate {
                                eprintln!("Setting PermanentDelegate delegate: {}", delegate);
                                OptionalNonZeroPubkey::try_from(Some(Token2022Pubkey::new_from_array(delegate.to_bytes())))
                                    .expect("Invalid permanent delegate")
                            } else {
                                eprintln!("Setting PermanentDelegate delegate to default (None)");
                                OptionalNonZeroPubkey::default()
                            };
                        }
                    } else {
                ...
                ExtensionType::GroupPointer => {
                    ...
                    let extension = state.init_extension::<GroupPointer>(true)
                        .expect("Failed to init group pointer extension");
                    
                    // Get the struct directly from ParamValue
                    if let Some(param) = extension_params.as_ref().and_then(|params| {
                        params.iter().find(|p| matches!(p, ParamValue::GroupPointer(_)))
                    }) {
                        if let ParamValue::GroupPointer(group_config) = param {
                            ...
                            // Set group_address
                            extension.group_address = if let Some(group_address) = group_config.group_address {
                                eprintln!("Setting GroupPointer group_address: {}", group_address);
                                OptionalNonZeroPubkey::try_from(Some(Token2022Pubkey::new_from_array(group_address.to_bytes())))
                                    .expect("Invalid group address")
                            } else {
                                eprintln!("Setting GroupPointer group_address to default (None)");
                                OptionalNonZeroPubkey::default()
                            };
                        }
                        
                        // Validate that at least one of authority or group_address is provided
                        if Option::<Pubkey>::from(extension.authority).is_none() && Option::<Pubkey>::from(extension.group_address).is_none() {
                ...
                ExtensionType::DefaultAccountState => {
                    ...
                    let extension = state.init_extension::<DefaultAccountState>(true)
                        .expect("Failed to init default account state extension");
                    
                    // Get the struct directly from ParamValue
                    if let Some(param) = extension_params.as_ref().and_then(|params| {
                        params.iter().find(|p| matches!(p, ParamValue::DefaultAccountState(_)))
                    }) {
                        if let ParamValue::DefaultAccountState(state_config) = param {
                            ...
                            eprintln!("Setting DefaultAccountState state: {}", state_config.state);
                ...
                ExtensionType::InterestBearingConfig => {
                    ...
                    let extension = state.init_extension::<InterestBearingConfig>(true)
                        .expect("Failed to init interest bearing config extension");
                    
                    // Get the struct directly from ParamValue
                    if let Some(param) = extension_params.as_ref().and_then(|params| {
                        params.iter().find(|p| matches!(p, ParamValue::InterestBearingConfig(_)))
                    }) {
                        if let ParamValue::InterestBearingConfig(interest_config) = param {
                            // Set rate_authority
                            extension.rate_authority = if let Some(rate_authority) = interest_config.rate_authority {
                                eprintln!("Setting InterestBearingConfig rate_authority: {}", rate_authority);
                                OptionalNonZeroPubkey::try_from(Some(Token2022Pubkey::new_from_array(rate_authority.to_bytes())))
                            ...
                            eprintln!("Setting InterestBearingConfig current_rate: {} basis points", interest_config.current_rate);
                            
                            // Set pre_update_average_rate
                            extension.pre_update_average_rate = interest_config.pre_update_average_rate.into();
                            ...
                            extension.initialization_timestamp = interest_config.initialization_timestamp.into();
                            eprintln!("Setting InterestBearingConfig initialization_timestamp: {}", interest_config.initialization_timestamp);
                            
                            // Set last_update_timestamp
                            extension.last_update_timestamp = interest_config.last_update_timestamp.into();
                            eprintln!("Setting InterestBearingConfig last_update_timestamp: {}", interest_config.last_update_timestamp);
                        }
                    } else {
                ...
                ExtensionType::TransferFeeConfig => {
                    ...
                    let extension = state.init_extension::<TransferFeeConfig>(true)
                        .expect("Failed to init transfer fee config extension");
                    
                    // Set parameters if provided
                    if let Some(param) = extension_params.as_ref().and_then(|params| {
                        params.iter().find(|p| matches!(p, ParamValue::TransferFeeConfig(_)))
                    }) {
                        if let ParamValue::TransferFeeConfig(fee_config) = param {
                            // Set transfer_fee_config_authority
                            extension.transfer_fee_config_authority = if let Some(transfer_fee_config_authority) = fee_config.transfer_fee_config_authority {
                                eprintln!("Setting TransferFeeConfig transfer_fee_config_authority: {}", transfer_fee_config_authority);
                                OptionalNonZeroPubkey::try_from(Some(Token2022Pubkey::new_from_array(transfer_fee_config_authority.to_bytes())))
                            ...
                            extension.withdraw_withheld_authority = if let Some(withdraw_withheld_authority) = fee_config.withdraw_withheld_authority {
                                eprintln!("Setting TransferFeeConfig withdraw_withheld_authority: {}", withdraw_withheld_authority);
                                OptionalNonZeroPubkey::try_from(Some(Token2022Pubkey::new_from_array(withdraw_withheld_authority.to_bytes())))
                            ...
                            extension.older_transfer_fee = TransferFee {
                                ...
                                transfer_fee_basis_points: fee_config.older_transfer_fee.transfer_fee_basis_points.into(),
                            };
                            ...
                            extension.newer_transfer_fee = TransferFee {
                                ...
                                transfer_fee_basis_points: fee_config.newer_transfer_fee.transfer_fee_basis_points.into(),
                            };
                    ...
                    ExtensionType::NonTransferable => {
                        ...
                        let _extension = state.init_extension::<NonTransferable>(true)
                    ...
                    ExtensionType::ConfidentialTransferMint => {
                        ...
                        let extension = state.init_extension::<ConfidentialTransferMint>(true)
                            .expect("Failed to init confidential transfer mint extension");
                        
                        // Set parameters if provided
                        if let Some(param) = extension_params.as_ref().and_then(|params| {
                            params.iter().find(|p| matches!(p, ParamValue::ConfidentialTransferMint(_)))
                        }) {
                            if let ParamValue::ConfidentialTransferMint(ct_config) = param {
                                // Set authority
                                extension.authority = if let Some(authority) = ct_config.authority {
                                    eprintln!("Setting ConfidentialTransferMint authority: {}", authority);
                                    OptionalNonZeroPubkey::try_from(Some(Token2022Pubkey::new_from_array(authority.to_bytes())))
                                ...
                                extension.auto_approve_new_accounts = ct_config.auto_approve_new_accounts.into();
                                ...
                                extension.auditor_elgamal_pubkey = if let Some(auditor_pubkey) = ct_config.auditor_elgamal_pubkey {
                                    use spl_pod::optional_keys::OptionalNonZeroElGamalPubkey;
                                    eprintln!("Setting ConfidentialTransferMint auditor_elgamal_pubkey");
                    ...
                    ExtensionType::ConfidentialTransferFeeConfig => {
                        use spl_token_2022::extension::confidential_transfer_fee::{ConfidentialTransferFeeConfig, EncryptedWithheldAmount};
         
                        let extension = state.init_extension::<ConfidentialTransferFeeConfig>(true)
                            .expect("Failed to init confidential transfer fee config extension");
                        
                        // Set parameters if provided
                        if let Some(param) = extension_params.as_ref().and_then(|params| {
                            params.iter().find(|p| matches!(p, ParamValue::ConfidentialTransferFeeConfig(_)))
                        }) {
                            if let ParamValue::ConfidentialTransferFeeConfig(ctf_config) = param {
                                ...
                                extension.authority = if let Some(authority) = ctf_config.authority {
                                    eprintln!("Setting ConfidentialTransferFeeConfig authority: {}", authority);
                                    OptionalNonZeroPubkey::try_from(Some(Token2022Pubkey::new_from_array(authority.to_bytes())))
                                ...
                                extension.withdraw_withheld_authority_elgamal_pubkey = ctf_config.withdraw_withheld_authority_elgamal_pubkey;
                                ...
                                extension.harvest_to_mint_enabled = ctf_config.harvest_to_mint_enabled.into();
                    ...
                    ExtensionType::ConfidentialMintBurn => {
                        ...
                        let extension = state.init_extension::<ConfidentialMintBurn>(true)
                            .expect("Failed to init confidential mint burn extension");
                        
                        // Set parameters if provided
                        if let Some(param) = extension_params.as_ref().and_then(|params| {
                            params.iter().find(|p| matches!(p, ParamValue::ConfidentialMintBurn(_)))
                        }) {
                    ...
                    _ => {
                        eprintln!("Extension type {:?} has no custom initialization logic", extension_type);
                    },
                }
            }
            ...
            let mint = spl_token_2022::state::Mint {
                is_initialized: true,
                mint_authority: COption::Some(Token2022Pubkey::new_from_array(mint_authority.to_bytes())),
