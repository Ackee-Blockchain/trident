...
impl AccountsStorage {
    ...
    pub(crate) fn create_token_2022_account(
        ...
        let actual_extensions: Vec<ExtensionType> = extension_params
            ...
            .map(|params| {
                params.iter().map(|param| match param {
                    ParamValue::ConfidentialTransferAccount(_) => ExtensionType::ConfidentialTransferAccount,
                    ...
                    ParamValue::NonTransferableAccount(_) => ExtensionType::NonTransferableAccount,
                    ParamValue::ConfidentialTransferFeeAmount(_) => ExtensionType::ConfidentialTransferFeeAmount,
        ...
        let account_size = if actual_extensions.is_empty() {
            ...
            ExtensionType::try_calculate_account_len::<spl_token_2022::state::Account>(&actual_extensions)
        ...
        if !actual_extensions.is_empty() {
            ...
            let mut state = StateWithExtensionsMut::<spl_token_2022::state::Account>::unpack_uninitialized(&mut data)
            ...
            for extension_type in &actual_extensions {
                ...
                ExtensionType::ConfidentialTransferAccount => {
                    use spl_token_2022::extension::confidential_transfer::{ConfidentialTransferAccount, EncryptedBalance};
                    use solana_zk_sdk::encryption::pod::auth_encryption::PodAeCiphertext;
                    let extension = state.init_extension::<ConfidentialTransferAccount>(true)
                    ...
                    if let Some(param) = extension_params.as_ref().and_then(|params| {
                        params.iter().find(|p| matches!(p, ParamValue::ConfidentialTransferAccount(_)))
                    }) {
                        if let ParamValue::ConfidentialTransferAccount(ct_config) = param {
                            // Set all required fields
                            extension.approved = ct_config.approved.into();
                            eprintln!("Setting ConfidentialTransferAccount approved: {}", ct_config.approved);
                            ...
                            extension.decryptable_available_balance = ct_config.decryptable_available_balance;
                            eprintln!("Setting ConfidentialTransferAccount decryptable_available_balance");
                            
                            extension.allow_confidential_credits = ct_config.allow_confidential_credits.into();
                            eprintln!("Setting ConfidentialTransferAccount allow_confidential_credits: {}", ct_config.allow_confidential_credits);
                            
                            extension.allow_non_confidential_credits = ct_config.allow_non_confidential_credits.into();
                            eprintln!("Setting ConfidentialTransferAccount allow_non_confidential_credits: {}", ct_config.allow_non_confidential_credits);
                            
                            extension.pending_balance_credit_counter = ct_config.pending_balance_credit_counter.into();
                            eprintln!("Setting ConfidentialTransferAccount pending_balance_credit_counter: {}", ct_config.pending_balance_credit_counter);
                            
                            extension.maximum_pending_balance_credit_counter = ct_config.maximum_pending_balance_credit_counter.into();
                            ...
                            extension.expected_pending_balance_credit_counter = ct_config.expected_pending_balance_credit_counter.into();
                            ...
                            extension.actual_pending_balance_credit_counter = ct_config.actual_pending_balance_credit_counter.into();
                    ...
                    ExtensionType::NonTransferableAccount => {
                        ...
                        let _extension = state.init_extension::<NonTransferableAccount>(true)
                        ...
                        eprintln!("Setting NonTransferableAccount extension (no parameters needed)");
                    },
                    ExtensionType::TransferFeeAmount => {
                        ...
                        // Set parameters if provided
                        if let Some(param) = extension_params.as_ref().and_then(|params| {
                            params.iter().find(|p| matches!(p, ParamValue::TransferFeeAmount(_)))
                        }) {
                            if let ParamValue::TransferFeeAmount(fee_amount_config) = param {
                                // Set withheld_amount directly (no longer Option)
                                extension.withheld_amount = fee_amount_config.withheld_amount.into();
                                eprintln!("Setting TransferFeeAmount withheld_amount: {}", fee_amount_config.withheld_amount);
                            }
                        } else {
                    ...
                    ExtensionType::MemoTransfer => {
                        ...
                        let extension = state.init_extension::<MemoTransfer>(true)
                            .expect("Failed to init memo transfer extension");
                        
                        // Set parameters if provided
                        if let Some(param) = extension_params.as_ref().and_then(|params| {
                            params.iter().find(|p| matches!(p, ParamValue::MemoTransfer(_)))
                        }) {
                            if let ParamValue::MemoTransfer(memo_config) = param {
                                // Set require_incoming_transfer_memos directly (no longer Option)
                                extension.require_incoming_transfer_memos = memo_config.require_incoming_transfer_memos.into();
                                eprintln!("Setting MemoTransfer require_incoming_transfer_memos: {}", memo_config.require_incoming_transfer_memos);
                            }
                        } else {
                    ...
                    ExtensionType::CpiGuard => {
                        ...
                        let extension = state.init_extension::<CpiGuard>(true)
                    ...
                    ExtensionType::ImmutableOwner => {
                        ...
                        let _extension = state.init_extension::<ImmutableOwner>(true)
                    ...
                    ExtensionType::ConfidentialTransferFeeAmount => {
                        use spl_token_2022::extension::confidential_transfer_fee::{ConfidentialTransferFeeAmount, EncryptedWithheldAmount};
                        
                        let extension = state.init_extension::<ConfidentialTransferFeeAmount>(true)
                            .expect("Failed to init confidential transfer fee amount extension");
                        
                        // Set parameters if provided
                        if let Some(param) = extension_params.as_ref().and_then(|params| {
                            params.iter().find(|p| matches!(p, ParamValue::ConfidentialTransferFeeAmount(_)))
                        }) {
                            if let ParamValue::ConfidentialTransferFeeAmount(fee_amount_config) = param {
                                // Set withheld_amount
                    ...
                    ExtensionType::TransferHookAccount => {
                        ...
                        // Set parameters if provided
                        if let Some(param) = extension_params.as_ref().and_then(|params| {
                            params.iter().find(|p| matches!(p, ParamValue::TransferHookAccount(_)))
                        }) {
                            if let ParamValue::TransferHookAccount(hook_config) = param {
                                ...
                                if let Some(transferring) = hook_config.transferring {
                                    ...
                                    eprintln!("Setting TransferHookAccount transferring: {}", transferring);
                                } else {
                    ...
                    _ => {
                        eprintln!("Extension type {:?} has no custom initialization logic yet", extension_type);
                    },
                }
            }
