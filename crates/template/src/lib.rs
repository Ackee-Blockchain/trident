...
impl TridentTemplates {
    ...
    fn process_accounts(
        ...
        // Sort accounts by dependency order
        let sorted_main_accounts = self.sort_accounts_by_dependencies(main_accounts);

        (sorted_main_accounts, composite_accounts)
    }
    ...
    /// Sort accounts by dependency order (accounts with dependencies come after their dependencies)
    fn sort_accounts_by_dependencies(
        &self,
        ...
        let mut account_map: HashMap<String, serde_json::Value> = HashMap::new();

        // First pass: collect all accounts and their dependencies
        for account in &accounts {
            let name = account
                .get("name")
                .and_then(|v| v.as_str())
                .unwrap_or_default()
                .to_string();
            account_map.insert(name.clone(), account.clone());

            let mut deps = HashSet::new();

            // Check account_seeds for dependencies
            if let Some(account_seeds) = account.get("account_seeds").and_then(|v| v.as_array()) {
                for seed in account_seeds {
                    if let Some(dep_name) = seed.as_str() {
        ...
        for (name, deps) in &dependencies {
            for dep in deps {
                if account_map.contains_key(dep) {
                    *in_degree.entry(name.clone()).or_insert(0) += 1;
                }
            }
        }

        // Find accounts with no dependencies
        ...
        while let Some(current) = queue.pop() {
            if let Some(account) = account_map.get(&current) {
                sorted.push(account.clone());
            }

            // Reduce in-degree of dependent accounts
            for (name, deps) in &dependencies {
        ...
        // If we haven't sorted all accounts, there might be circular dependencies
        // In that case, append remaining accounts in original order
        if sorted.len() < accounts.len() {
            for account in &accounts {
                let name = account
                    .get("name")
                    .and_then(|v| v.as_str())
                ...
                if !sorted
                    .iter()
                    .any(|a| a.get("name").and_then(|v| v.as_str()).unwrap_or_default() == name)
                {
                    sorted.push(account.clone());
                }
            }
