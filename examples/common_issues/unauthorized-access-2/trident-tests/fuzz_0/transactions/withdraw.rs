...
impl TransactionCustomMethods for WithdrawTransaction {
    fn transaction_invariant_check(&self) -> Result<(), FuzzingError> {
        if let Ok(escrow_pre) = Escrow::deserialize(&mut self.instruction.accounts.escrow.get_snapshot_before().data_no_discriminator()) {
            let receiver_key = self.instruction.accounts.receiver.get_snapshot_before().pubkey();
            let receiver_lamports_before = self.instruction.accounts.receiver.get_snapshot_before().lamports();
            let receiver_lamports_after = self.instruction.accounts.receiver.get_snapshot_after().lamports();
