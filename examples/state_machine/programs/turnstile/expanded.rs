...
pub mod turnstile {
    ...
    pub fn coin(ctx: Context<UpdateState>, dummy_arg: String)
     -> ProgramResult {
        let state = &mut ctx.accounts.state;
        state.locked = false;
        Ok(())
...
pub mod instruction {
    ...
    impl borsh::ser::BorshSerialize for Coin where
    ...
    impl borsh::de::BorshDeserialize for Coin where
     String: borsh::BorshDeserialize {
        fn deserialize(buf: &mut &[u8])
         -> ::core::result::Result<Self, borsh::maybestd::io::Error> {
            Ok(Self{dummy_arg: borsh::BorshDeserialize::deserialize(buf)?,})
        }
    }
